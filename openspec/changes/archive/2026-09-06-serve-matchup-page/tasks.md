...
- [x] 1.1 Move the season and week bound constants (`minSeason`, `maxSeason`, `minWeek`, `maxWeek`,
...
- [x] 2.1 Create `internal/web`, declaring its own
...
- [x] 2.2 RED: test, through an `http.ServeMux` registered as `GET /{season}/{week}`, that
      `GET /2025/fifteen` responds `400`. Run; confirm it fails on the `501`.
- [x] 2.3 GREEN: parse both segments with `strconv.Atoi` and answer `400` when either fails. Re-run;
      confirm pass. Confirm the well-formed case still reaches the stub.
- [x] 2.4 RED: test that `GET /2025/23` and `GET /1998/3` each respond `400`. Run; confirm they
      currently pass the range check that does not exist yet.
- [x] 2.5 GREEN: range-check against the shared bounds from 1.1. Re-run; confirm pass.
- [x] 2.6 RED: test that a `400` request opens no roster file and makes no upstream request —
...
- [x] 2.7 GREEN: confirm the validation returns before both. Comment why the order matters: a typo
      in a URL must not reach Sleeper.
- [x] 2.8 RED: test that `POST /2025/15` responds `405`. Run; confirm the mux, not the handler,
      produces it — if it already passes, keep it as the test that pins the method-qualified
      pattern.

## 3. Resolving the week into two columns

- [x] 3.1 RED: test that a `t.TempDir()` week holding `bojjaes.csv` and `wood.csv` renders a `200`
      whose body contains both team names, with `bojjaes` appearing before `wood`. Run; confirm it
      fails on the `501`.
- [x] 3.2 GREEN: call `tree.Matchup`, build a two-column view model, and render it through a
...
- [x] 3.3 RED: test that a week whose opponent sorts before us (`aardvarks.csv`) still renders
...
- [x] 3.4 RED: test that a week directory that does not exist responds `404`. Run; confirm it is
      currently a `200` or a `500`.
- [x] 3.5 GREEN: map `roster.ErrNoWeek` with `errors.Is` to `404`. Re-run; confirm pass.
- [x] 3.6 RED: test that a week holding three rosters responds `500`, and that the body is not a
      scoreboard. Run; confirm.
- [x] 3.7 GREEN: map the remaining matchup sentinels — too many, too few, not ours — to `500`.
      ...
      well-formed request.
- [x] 3.8 RED: test that a week holding two rosters, neither of them ours, responds `500`. Run.
- [x] 3.9 GREEN: confirm pass.
- [x] 3.10 RED: test that a week whose `wood.csv` has a line with no id responds `500` and renders
      no page. Run; confirm the parse error is not currently surfaced as a `200`.
- [x] 3.11 GREEN: refuse an unreadable roster with `500`. Re-run; confirm pass.
- [x] 3.12 Confirm each of the four refusals logs server-side with the failing path or directory,
      and that the response body carries no filesystem path.

## 4. Scored starters and totals

- [x] 4.1 RED: test that a week whose rosters hold ids present in the stub payload renders each
      starter's **name** from the CSV alongside its points. Run; confirm the points are absent or
      wrong.
- [x] 4.2 GREEN: score each starter through `Week.Player` and `Points`, formatting the value into
      the view model. Re-run; confirm pass.
- [x] 4.3 RED: test that a column of nine known scorers renders the total of their points. Run;
      confirm it fails on the total.
- [x] 4.4 GREEN: sum the starters into the column's total. Re-run; confirm pass.
- [x] 4.5 RED: test that a roster of twelve records renders only the first nine, and that the body
      contains none of the last three names. Run; confirm bench players currently appear or the
      total is wrong.
- [x] 4.6 GREEN: take `Roster.Starters()`. Re-run; confirm pass.
- [x] 4.7 RED: test that the `StatsSource` is called exactly **once** for a request that renders
      both columns — count calls in the fake. Run; confirm it is currently two.
- [x] 4.8 GREEN: fetch once and score both columns from the same `WeekStats`. Re-run; confirm pass.
      Comment that the two columns must not be read from different snapshots of the week.
- [x] 4.9 RED: test that a `StatsSource` returning an error makes the page respond `502` with no
      scoreboard in the body. Run; confirm.
- [x] 4.10 GREEN: map a fetch failure to `502`. Re-run; confirm pass. Comment that a zeroed page
      would read as "these players scored nothing" rather than "we do not know".

## 5. Starters the provider has nothing for

- [x] 5.1 RED: test that a starter absent from the payload renders `no stats` and **not** `0`. Run;
      confirm a `0` is currently rendered.
- [x] 5.2 GREEN: carry the placeholder in the view model, decided in Go rather than in the
      template. Re-run; confirm pass. Comment that `no stats` matches `scripts/scores.sh` while
      both UIs exist.
- [x] 5.3 RED: test that a column with one absent starter and eight scorers totalling 40 renders a
      total of 40. Run; confirm.
- [x] 5.4 GREEN: skip absent starters when summing. Re-run; confirm pass.
- [x] 5.5 RED: test that a starter **present** in the payload with no scoring production renders
      `0` and not `no stats`. Run; confirm this distinguishes absence from a scoreless week.
- [x] 5.6 GREEN: make it pass; confirm the two cases are decided by `Player`'s second return value
      and nothing else.

## 6. A page that implies no winner

- [x] 6.1 RED: test that a page whose columns total 56 and 42 contains neither `14` as a rendered
      value nor any element carrying a difference. Run; confirm.
- [x] 6.2 GREEN: confirm the view model has no margin field and the template computes nothing.
      Re-run.
- [x] 6.3 Add the `<style>` block: a two-column grid, both columns equal width and sharing one
...
- [x] 6.4 RED: test that the two columns' markup differs only in their content — assert the same
      class appears exactly twice and that no class name in the body matches a leader/winner
      vocabulary. Run; confirm.
- [x] 6.5 GREEN: make it pass.
- [x] 6.6 RED: test that a roster record whose name contains `<b>` renders escaped, and that the
...
- [x] 6.7 Confirm the response sets an HTML content type, and that the handler renders into a
      `bytes.Buffer` and writes only on success — add a test if the failure path can be provoked,
      otherwise confirm by reading.

## 7. Serving it

- [x] 7.1 Register `GET /{season}/{week}` in `cmd/server/main.go`, constructing the lineup tree
...
- [x] 7.2 Confirm `POST /scores` still responds as before — run the existing suite and hit it by
      hand against a locally running server.
- [x] 7.3 Run the server against the real tree and load `/2025/14`, `/2025/15`, and `/2025/16` in a
...
- [x] 7.4 Update the `main.go` package comment and the README to describe the page route alongside
      `POST /scores`.

## 8. Close out

- [x] 8.1 Re-read the new code as a whole and delete any comment that restates it; keep the ones
...
- [x] 8.2 Run `gofmt -l ./...`, `go vet ./...`, and `go test ./...`; confirm clean and that
      `scripts/**` and the roster CSV format are untouched.
- [x] 8.3 Walk the `matchup-page` spec scenario by scenario and confirm each has a test that
      exercises it; add any the loop above did not produce.
- [x] 8.4 Tick the backlog's "Add `GET /{season}/{week}`…" line.
