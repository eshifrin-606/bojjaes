...
- [ ] 1.1 In `internal/statscache`, add `WeekStatsAsOf(ctx, season, week) (score.WeekStats,
...
- [ ] 1.2 Update the compile-time interface assertions in the `statscache` test file to also assert
      `*Cache` satisfies a local copy of `web`'s new `WeekStatsAsOf` interface shape, alongside the
      existing `api`/`web` `WeekStats` one.
- [ ] 1.3 In `internal/web`, change the local `StatsSource` interface to the single method
...
- [ ] 1.4 Confirm `go build ./...` and `go vet ./...` are clean and `cmd/server/main.go` is
...
- [ ] 2.1 RED: in `internal/statscache`, test that `WeekStatsAsOf` on a fresh miss returns a
      non-zero instant equal to the clock at fetch completion. Run; confirm it fails reporting the
      zero instant, against 1.1.
- [ ] 2.2 GREEN: return `entry.fetchedAt` from the miss path. Re-run; confirm pass and confirm the
      `weekly-stats-cache` suite still passes.
- [ ] 2.3 RED: test that a cache hit returns the **original** fetch instant — fetch, advance the
      ...
      and restore.
- [ ] 2.4 RED: test that every waiter on one in-flight fetch receives the same completion instant —
...
- [ ] 2.5 GREEN: return `cached.fetchedAt` from the waiter path (it is set under `c.mu` before
      `close(flight.done)`, so a released waiter reads a stable value). Re-run 2.1–2.4; confirm all
      pass.
- [ ] 2.6 RED: regression — test that `WeekStats` (no `AsOf`) still returns exactly `(stats, err)`
...
- [ ] 3.1 Add `America/Chicago` loading to `internal/web`: `time.LoadLocation` at package init
...
- [ ] 3.2 RED: test that a served page contains exactly one `<time>` element whose `datetime`
...
- [ ] 3.3 GREEN: add `FetchedAtRFC3339 string` to the view model, set it from the handler's
...
- [ ] 3.4 RED: test that the RFC 3339 attribute denotes the same instant regardless of the fetch's
...
- [ ] 4.1 RED: test that the page's visible text (tags stripped) contains the `fetchedAt` instant
...
- [ ] 4.2 GREEN: add `FetchedAtText string` to the view model, formatted from
      ...
      Re-run; confirm pass.
- [ ] 4.3 RED: test the label — the visible text around the `<time>` element identifies the instant
...
- [ ] 4.4 GREEN: add the label text in the template (e.g. `Sleeper stats fetched <time …>…</time>`).
      Re-run; confirm pass.
- [ ] 4.5 RED: test the no-winner constraint for the new element — render the spec's 56 vs 42
...
- [ ] 5.1 RED: a `internal/web` handler test wired to a real `statscache.Cache` (injected clock)
...
- [ ] 5.2 RED: test that the second request in 5.1 made **no** second upstream call (fake's count
      is 1) — the as-of path did not bypass the cache. Run; confirm.

## 6. Docs and close-out

- [ ] 6.1 Update `docs/package-dependencies.md` if it states the `web`→`statscache` interface
      shape or method name; the dependency direction and arrows are unchanged.
- [ ] 6.2 Tick the backlog line "Stamp the as-of from our own Sleeper fetch time for now, labelled
      ...
      `notes.md`.
- [ ] 6.3 Write `notes.md`: the deferred live-Sunday drift observation (procedure, what it feeds),
...
- [ ] 6.4 `go test ./...` clean; `openspec validate stamp-as-of-timestamp --strict` clean.
