...
Govern what the container image the server is deployed as must be: built from the server's own
package, a statically linked binary, a final stage holding nothing but that binary and the CA roots
it needs, run as a non-root user with the server as its init process, and a module download layer
keyed on both module files.
